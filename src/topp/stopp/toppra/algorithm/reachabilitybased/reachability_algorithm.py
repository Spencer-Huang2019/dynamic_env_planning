...
class ReachabilityAlgorithm(ParameterizationAlgorithm):
    ...
    def compute_parameterization(self, sd_start, sd_end, return_data=False):
        ...
        # ps = self.path(self.gridpoints, 1)
        # pss = self.path(self.gridpoints, 2)
        # psss = self.path(self.gridpoints, 3)
        #
        # np.savetxt('ps.txt', ps)
        # np.savetxt('pss.txt', pss)
        # np.savetxt('psss.txt', psss)
        # np.savetxt('xs.txt', xs)
        # np.savetxt('ss.txt', self.gridpoints)
