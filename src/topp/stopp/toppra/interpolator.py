...
def propose_gridpoints(
        path, max_err_threshold=2e-2, max_iteration=50, max_seg_length=0.1,
        min_nb_points=20
):
...
class SplineInterpolator(AbstractGeometricPath):
    ...
    def __init__(self, ss_waypoints, waypoints, start_qs=None, bc_type="not-a-knot"):
        ...
        if len(ss_waypoints) == 1:
            ...
            self.csplddd = _1dof_cspld
        else:
            if start_qs is None:
                self.cspl = CubicSpline(ss_waypoints, waypoints, bc_type=bc_type)
            else:
                self.cspl_ = CubicSpline(ss_waypoints, waypoints, bc_type=bc_type)
                self.cspld_ = self.cspl_.derivative()
                self.cspl = CubicSpline(ss_waypoints, waypoints,
                                        bc_type=((1, start_qs), (1, self.cspld_(ss_waypoints[-1]))))
