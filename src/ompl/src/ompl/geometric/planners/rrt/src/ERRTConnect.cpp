//
// Created by HP on 2022/12/29.
//

#include "ompl/geometric/planners/rrt/ERRTConnect.h"
#include "ompl/base/goals/GoalSampleableRegion.h"
#include "ompl/tools/config/SelfConfig.h"
#include "ompl/util/String.h"

ompl::geometric::ERRTConnect::ERRTConnect(const base::SpaceInformationPtr &si, bool addIntermediateStates)
  : base::Planner(si, addIntermediateStates ? "ERRTConnectIntermediate" : "ERRTConnect")
{
    specs_.recognizedGoal = base::GOAL_SAMPLEABLE_REGION;
    specs_.directed = true;

    Planner::declareParam<double>("range", this, &ERRTConnect::setRange, &ERRTConnect::getRange, "0.:1.:10000.");
    Planner::declareParam<bool>("intermediate_states", this, &ERRTConnect::setIntermediateStates,
                                &ERRTConnect::getIntermediateStates, "0,1");
...
ompl::geometric::ERRTConnect::GrowState ompl::geometric::ERRTConnect::growTree(TreeData &tree, TreeGrowingInfo &tgi,
                                                                             Motion *rmotion)
{
...
{
    OMPL_INFORM("%s: solve solution by ERRTConnect", getName().c_str());
    checkValidity();
            ...
            while (!ptc) {
                ...
                if (gs != TRAPPED) {
                    ...
                    if (gsc == REACHED && goal->isStartGoalPairValid(startMotion->root, goalMotion->root)) {
                        ...
                        pdef_->addSolutionPath(index, path, false, 0.0, getName());
                        solved = true;
                        firstSolution = true;
//                        OMPL_INFORM("%s: start states count %u", getName().c_str(), index);
                        break;
                    } else {
            ...
            if (approxsol && !solved) {
                OMPL_INFORM("%s: Get approxsol result", getName().c_str());
                /* construct the solution path */
                ...
                pdef_->addSolutionPath(index, path, true, approxdif, getName());
                firstSolution = true;
                approxSolved = true;
...
//        {
//            OMPL_INFORM("%s: add another start state solution", getName().c_str());
////            auto latterPath(std::make_shared<PathGeometric>(si_));
//            index++;
//            PathGeometric latterPath = static_cast<PathGeometric &>(*pdef_->getSolutionPath());
//            OMPL_INFORM("latterPath length: %lu", latterPath.getStateCount());
//            auto path(std::make_shared<PathGeometric>(si_));
//            path->getStates().reserve(latterPath.getStateCount() + 1);
//            path->append(st);
//            path->append(latterPath);
//            OMPL_INFORM("Path length: %lu", path->getStateCount());
//            if (solved)
//                pdef_->addSolutionPath(index, path, false, 0.0, getName());
//            else
//                pdef_->addSolutionPath(index, path, true, approxdif, getName());
...
//    }
    ...
    OMPL_INFORM("%s: return result", getName().c_str());
