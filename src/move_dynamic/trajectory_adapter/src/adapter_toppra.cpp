...
#include <move_dynamic/trajectory_adapter/adapter_toppra.h>
...
{
AdapterToppra::AdapterToppra(const double resample_dt, const double min_angle_change) : resample_dt_(resample_dt),
min_angle_change_(min_angle_change)
{
}

bool AdapterToppra::runToppra(robot_trajectory::RobotTrajectory& trajectory, const double max_velocity_scaling_factor,
                         const double max_acceleration_scaling_factor, double init_path_velocity)
{
    ...
    ROS_INFO_NAMED(LOGNAME, "before toppra::Vector");
    toppra::Vector vel_limit_lower = Eigen::VectorXd::Zero(num_joints_);
    ROS_INFO_NAMED(LOGNAME, "after toppra::Vector");
...
toppra::Vector AdapterToppra::getSampleTimes(std::size_t sample_count, double duration)
{
    toppra::Vector times{sample_count + 1};
    for (size_t sample = 0; sample <= sample_count; ++sample)
    {
        double t = std::min(duration, sample * resample_dt_);
        times[sample] = t + times[sample - 1];
    }

    return times;
}

toppra::Vector AdapterToppra::getPathPositions(toppra::Vectors& points)
{
    toppra::Vector path_positions{num_points_};
    path_positions[0] = 0.0;
    for (size_t p = 1; p < num_points_; ++p)
    {
        double dist = 0.0;
