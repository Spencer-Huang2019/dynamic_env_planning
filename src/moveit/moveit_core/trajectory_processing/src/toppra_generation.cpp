...
bool ToppraGeneration::computeTrajectory(robot_trajectory::RobotTrajectory& trajectory,
                                         ...
                                         moveit::core::RobotStatePtr start_state_ptr)
{
    ...
    if (start_state_ptr)
    {
        ...
        if (!callToppraPython(points, path_positions, vel_lim, acc_lim, resample_dt_, init_path_velocity, init_qs))
        {
            return false;
        }
    }
    ...
    {
        if (!callToppraPython(points, path_positions, vel_lim, acc_lim, resample_dt_))
        {
            return false;
        }
    }
...
bool ToppraGeneration::callToppraPython(std::vector<std::vector<double>>& points, std::vector<double>& path_positions,
                                        std::vector<std::vector<double>>& vel_lim, std::vector<std::vector<double>>& acc_lim,
                                        double resample_dt, double init_path_velocity, std::vector<double> init_qs)
{
    ROS_INFO_NAMED(LOGNAME, "run TOPPRA python");
...
//    py::scoped_interpreter python;
    ...
    py::object result = toppra.attr("call_toppra")(points,
            path_positions, vel_lim, acc_lim, resample_dt, init_path_velocity, init_qs);
    py::list result_list = result.cast<py::list>();
    ROS_INFO_NAMED(LOGNAME, "TOPPRA finish computation");
...
std::vector<double> ToppraGeneration::getSampleTimes(std::size_t sample_count, double duration)
{
...
std::vector<double> ToppraGeneration::getPathPositions(std::vector<std::vector<double>>& points)
{
