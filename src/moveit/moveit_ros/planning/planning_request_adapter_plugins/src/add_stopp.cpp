...
class AddStopp : public planning_request_adapter::PlanningRequestAdapter
{
public:
    AddStopp() : planning_request_adapter::PlanningRequestAdapter()
  {
  }
                    ...
                    std::vector<std::size_t>& /*added_path_index*/) const override
  {
    ...
    if (result && res.trajectory_)
    {
        ...
        StoppGeneration stopp(getSampleDuration(), 0.001);
        if (!stopp.computeTrajectory(*res.trajectory_, req.max_vel, req.max_acc, req.max_jerk,
                      ...
                      std::vector<std::size_t>& /*added_path_index*/) const override
    {
        ...
        if (result && res.trajectories_[start_index])
        {
            ...
            StoppGeneration stopp(getSampleDuration(), 0.001);
            if (!stopp.computeTrajectory(*res.trajectories_[start_index], req.max_vel, req.max_acc, req.max_jerk,
                                         req.max_velocity_scaling_factor, req.max_acceleration_scaling_factor,
                                         start_state_ptr)) {
                ROS_ERROR_NAMED(LOGNAME, "Time parametrization for the solution path failed.");
            }
...
CLASS_LOADER_REGISTER_CLASS(default_planner_request_adapters::AddStopp,
